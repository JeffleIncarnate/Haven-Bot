...
class Talk(commands.Cog):
    ...
    async def never_gonna_give_you_up(self, ctx):
        ...
        embed.set_thumbnail(
            url="https://variety.com/wp-content/uploads/2021/07/Rick-Astley-Never-Gonna-Give-You-Up.png?w=681&h=383&crop=1"
        )
    ...
    async def e(self, ctx):
        embed = discord.Embed(
            title="EEEEEEEEEEEEEEEE", color=discord.Colour.from_rgb(66, 135, 245)
        )
        await ctx.respond(embed=embed)
    ...
    async def ping(self, ctx):
        embed = discord.Embed(
            title="Pong!", description=f"{round(self.bot.latency * 1000)}ms of ping!"
        )
        await ctx.respond(embed=embed)
