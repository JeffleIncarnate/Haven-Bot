...
class Bedwards(commands.Cog):
    ...
    @discord.slash_command(description="Get the Hypixel Bedwars stats of a user")
    async def bedwars_stats(self, ctx, username: str):
        ...
        get_stats_url = (
            f"https://api.hypixel.net/player?key={hypixel_token}&uuid={uuid}"
        )
        ...
        user_stats_dict = {
            ...
            "final_kills": user_stats["player"]["stats"]["Bedwars"][
                "final_kills_bedwars"
            ],
            "beds_broken": user_stats["player"]["stats"]["Bedwars"][
                "beds_broken_bedwars"
            ],
        }
